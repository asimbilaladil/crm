...
class Survey extends CI_Controller {
    ...
    public function index() {
        ...
        $publishToken = $this->uri->segment(2);
        $currentDate =  date("Y-m-d");

        $publish = $this->PublishModel->getByTokenAndType($publishToken, 'public',  $currentDate);
        if(empty($publish)){
        ...
        $this->load->view('Public/Survey', array('data' => $data));
